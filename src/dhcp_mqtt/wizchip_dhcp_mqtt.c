...
#include "wizchip_overrides.h" // Наши значения пинов
...
/* Socket */
#define SOCKET_MQTT 1 // Чтобы не пересекался с DHCP
...
#define MQTT_PUBLISH_PERIOD (1000 * 10) // 60 seconds
...
static uint8_t g_mqtt_broker_ip[4] = {192, 168, 1, 238};
...
void mqtt_task(void *argument)
{
    ...
    // task_dns тоже инициализирует сеть, поэтому не трогаем её повторно.
    // network_initialize(g_net_info);
...
static void repeating_timer_callback_DHCP(void)
{
    g_msec_cnt++;
...
static void repeating_timer_callback(void)
{
    /* FIXME: Если что-то пойдёт не так, надо убрать MilliTimer_Handler -- mish */
    MilliTimer_Handler();

    repeating_timer_callback_DHCP();
}
